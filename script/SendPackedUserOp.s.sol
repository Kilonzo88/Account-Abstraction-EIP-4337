...
contract SendPackedUserOp is Script, HelperConfig {
    using MessageHashUtils for bytes32;

    function run() public {
        HelperConfig helperConfig = new HelperConfig();
        address destination = helperConfig.getConfig().usdc;
        uint256 value = 0;
        address minimalAccountAddress = DevOpsTools.get_most_recent_deployment("MinimalAccount", block.chainid);
        address TRUSTED_ACCOUNT = 0x52192F31214108c4feD5e00Ee07A9278538e0561;
        bytes memory functionData = abi.encodeWithSelector(IERC20.approve.selector, TRUSTED_ACCOUNT, 1e18);
        bytes memory executeCallData = abi.encodeWithSelector(MinimalAccount.execute.selector, destination, value, functionData);
        PackedUserOperation memory userOp = generateSignedUserOperations(executeCallData, helperConfig.getConfig(), minimalAccountAddress);
    ...
    function generateSignedUserOperations(bytes memory callData, HelperConfig.NetworkConfig memory config, address minimalAccount) public view returns(PackedUserOperation memory) {
    ...
    function _generateUnsignedUserOperation(bytes memory callData, address sender, uint256 nonce) internal pure returns(PackedUserOperation memory) {
        uint128 verificationGasLimit = 16777212;
